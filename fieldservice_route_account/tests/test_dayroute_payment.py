...
class TestDayrouteAccount(common.TransactionCase):
    def setUp(self):
        ...
        # Create a worker
        self.fsm_worker = self.env["fsm.person"].create({"name": "Test Worker"})
    ...
    def test_dayroute_account(self):
        ...
        for dayroute_payment in self.test_order.dayroute_id.dayroute_payment_ids:
            ...
            dayroute_close_state = self.env["fsm.stage"].search(
                ["&", ("stage_type", "=", "route"), ("is_closed", "=", True)]
            )
            # Closing Day Route
            self.test_order.dayroute_id.write({"stage_id": dayroute_close_state.id})
            # A journal entry will be created.
            self.assertTrue(dayroute_payment.move_id)
            self.assertEqual(
                dayroute_payment.move_id.amount, dayroute_payment.difference
            )
