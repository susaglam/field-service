...
class TestFSMSaleOrder(TestFSMSale):
    ...
    def test_sale_order_1(self):
        ...
        # The template's order type reaches the order, not only its instructions
    ...
    def test_prepare_fsm_values_order_type(self):
        """One sale-level order for several templates: the type is set only
        when the templates agree on one, and the old keywords still work."""
        other_type = self.env["fsm.order.type"].create({"name": "Other Type"})
        self.fsm_template_2.type_id = other_type
        order = self.sale_order_1

        vals = order._prepare_fsm_values(
            so_id=order.id, templates=self.fsm_template_1 | self.fsm_template_2
        )
        self.assertFalse(vals["template_id"])
        self.assertNotIn("type", vals)

        vals = order._prepare_fsm_values(
            so_id=order.id, templates=self.fsm_template_1 | self.fsm_template_3
        )
        self.assertEqual(vals["type"], self.fsm_order_type.id)

        # An override still calling with template_ids / template_id is honoured
        vals = order._prepare_fsm_values(
            so_id=order.id, template_ids=self.fsm_template_1.ids
        )
        self.assertEqual(vals["template_id"], self.fsm_template_1.id)
        self.assertEqual(vals["type"], self.fsm_order_type.id)
        vals = order._prepare_fsm_values(
            so_id=order.id, template_id=self.fsm_template_2.id
        )
        self.assertEqual(vals["type"], other_type.id)
