...
class ResCompany(models.Model):
    ...
    auto_populate_persons_on_location = fields.Boolean(
        ...
        help="Auto Populate Persons On Location. Automation toggle — when on, the "
        "related action happens automatically.",
    )
    auto_populate_equipments_on_order = fields.Boolean(
        string="Auto-populate Equipments on Order based on Location",
        help="Auto Populate Equipments On Order. Automation toggle — when on, the "
        "related action happens automatically.",
    )
    search_on_complete_name = fields.Boolean(
        string="Search Location By Hierarchy", help="Search On Complete Name."
    )
