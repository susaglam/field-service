...
class FSMStage(models.Model):
    ...
    def get_color_information(self):
        # every stage needs its colour in the map; stages are a short config list
    ...
    def create(self, vals_list):
        for vals in vals_list:
            # Ask for the clashing stage instead of loading every stage (W8163).
            # Both keys must be given, as the loop this replaces required.
            if (
                "stage_type" in vals
                and "sequence" in vals
                and self.search_count(
                    [
                        ("stage_type", "=", vals["stage_type"]),
