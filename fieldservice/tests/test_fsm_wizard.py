...
class FSMWizard(TransactionCase):
    ...
    def test_convert_sublocation(self):
        ...
        # (wiz_parent.child_ids are sub-LOCATIONS, not the partner's contacts)
        self.assertTrue(wiz_parent.partner_id.child_ids)
    ...
    def test_prepare_fsm_location_child_under_fsm_parent(self):
        ...
        child_partner = self.env["res.partner"].create(
            {"parent_id": self.test_partner.id, "name": "Child Under FSM Parent"}
        )
    ...
    def test_prepare_fsm_location_child_without_parent_location(self):
        child_partner = self.env["res.partner"].create(
            {"parent_id": self.test_parent_partner.id, "name": "Child No FSM Parent"}
        )
        vals = self.Wizard._prepare_fsm_location(child_partner)
        self.assertEqual(vals["owner_id"], self.test_parent_partner.id)
        self.assertNotIn("parent_id", vals)

    def test_convert_twice_explains_what_exists(self):
        self.Wizard.action_convert_location(self.test_partner)
        with self.assertRaisesRegex(UserError, "already a Field Service location"):
            self.Wizard.action_convert_location(self.test_partner)
