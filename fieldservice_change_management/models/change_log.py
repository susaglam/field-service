...
class ChangeLog(models.Model):
    ...
    implemented_on = fields.Datetime(
        required=True, default=fields.Datetime.now, help="Implemented On."
    )
    ...
    tag_ids = fields.Many2many(
        "change.log.tag",
        string="Tags",
        help="Tag Ids. Many-to-many / one-to-many relation collection.",
    )
    type_id = fields.Many2one(
        "change.log.type",
        string="Type",
        required=True,
        help="Type Id. Linked record reference.",
    )
    impact_id = fields.Many2one(
        "change.log.impact",
        string="Impact",
        required=True,
        help="Impact Id. Linked record reference.",
    )
